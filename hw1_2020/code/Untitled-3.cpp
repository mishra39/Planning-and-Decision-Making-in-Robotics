...
*/
pair<int,int> SearchPath(double* map,
...
{
    static pathSearch SearchObj(map, collision_thresh, x_size, y_size, robotposeX, robotposeY, target_steps, target_traj, targetposeX, targetposeY, curr_time, action_ptr);
    int retrnIdx;
    
    if (curr_time == 0)
    {
        SearchObj.initNewCell();
        SearchObj.computePath();
    }

    retrnIdx = GETMAPINDEX(robotposeX, robotposeY, x_size, y_size);
    mexPrintf("Returning Index %d\n", retrnIdx);
    mexPrintf("Map Size %d x %d\n", x_size, y_size);
    return (make_pair(SearchObj.actionMapX[retrnIdx], SearchObj.actionMapY[retrnIdx]));
}
static void planner(
        double*	map,
        int collision_thresh,
...
{   
    mexPrintf("Passing Map Size %d x %d\n", x_size, y_size);
    pair<int,int> moveXY = SearchPath(map, collision_thresh, x_size, y_size, robotposeX, robotposeY, target_steps, target_traj, targetposeX, targetposeY, curr_time, action_ptr);
    action_ptr[0] = moveXY.first;
    action_ptr[1] = moveXY.second;
