...
{   
    ...
    while(1)
    {
      if (currNode->idx > 0)
      {
        ...
        pathQuality += distanceCalc(currNode->jointAng, currNode->parent->jointAng);
     ...
     )
{
    ...
    while(closestToGoal->parent != NULL)
    {
      ...
      pathQuality += distanceCalc(closestToGoal->jointAng, closestToGoal->parent->jointAng);
    ...
    *planlength = 0;
    ...
    while (1)
    {
      if ((backNode->parent->idx == 0) || (backNode->parent->idx == V))
      {
        ...
        pathQuality += distanceCalc(backNode->jointAng, backNode->parent->jointAng);
        break;
      }
...
/*   distStart = distanceCalc(randConfig, startConfig);
    ...
    lastConfig = NULL;
    if (!isEdgeValidRRT(newConfigDist,nearestNode->jointAng, newConfig,lastConfig, map, x_size, y_size))
    {
      if (lastConfig == NULL)
      {
        continue;
    ...
    vector<Node*>* closestNodes = new vector<Node*>();
    vector<double>* closestDist = new vector<double>();
    findNeighbors(graph, closestNodes, closestDist, newConfig, epsilon); // find neighbors of new config
    if (closestNodes->size()>10)
    {
      int rad = min((gamma/armDOF)*(log(graph->size())/ graph->size()), epsilon);
      //printf("Epsilon is %.2f\n",epsilon);
      //printf("Search radius is: %.2f \n",rad);
    ...
    if (distanceCalc(newConfig, armgoal_anglesV_rad) < epsilon/2)
    {
      printf("Goal Found \n");
      break;
    }
