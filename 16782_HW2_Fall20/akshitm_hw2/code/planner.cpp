...
void randomConfig(double** config_arr,int numofDOFs)
{
  random_device rd;
...
static void findNeighbors(vector<Node*>* graph, vector<Node*>* nearestKNodes,
  vector<double>* nearestKDist, double* config, double nnRad, int armDOF) // i represents the total number of nodes in the roadmap so far
{
...
bool isEdgeValid(double neighborDist, double* currNodeAng, double* adjNodeAng, double *map, int x_size, int y_size, int armDOF)
{
...
bool isEdgeValidRRT(double neighborDist, double* Xnearest, double* Xnew,double* prevConfig, double *map, int x_size, int y_size, int armDOF)
{
...
double heuristicCalc(double* nodeConfig, double* goalConfig, int armDOF) // computes the distance between two configurations
{
  double heuristic = 0;
...
// Calculates the euclidean distance between two configurations in the C-space
double distanceCalc(double *configA, double* configB, int armDOF)
{
  double dist = 0;
  for (int i = 0; i < armDOF; i++)
  {
...
// Generates a configuration with a bias towards the goal region
void goalbiasConfig(double* goalConfig, double regionSize, double** randCOnfig, int armDOF)
{
  random_device rd;
  default_random_engine gen(rd());
  for (int i = 0 ; i < armDOF; i++)
...
{   
    ...
    while (i < V)
    {
      ...
      Node* adjNode;
      randomConfig(&currConfig,numofDOFs);
      ...
      kNNrad =  epsilon;
      findNeighbors(graph, nearestKNodes, nearestKDist,currConfig, kNNrad,numofDOFs);
      
      for (int ss=0; ss < nearestKNodes->size(); ss++)
      {
        adjNode = (*nearestKNodes)[ss];
        double neighborDist = (*nearestKDist)[ss];
        if (!isEdgeValid(neighborDist,currConfig, adjNode->jointAng, map,x_size, y_size,numofDOFs))
        {
    ...
    Node* neighborNode;
    findNeighbors(graph, closestNodes, closestDist,armstart_anglesV_rad, epsilon,numofDOFs);
    for (int ss=0; ss < closestNodes->size(); ss++)
    {
      neighborNode = (*closestNodes)[ss];
      double neighborDist = (*closestDist)[ss];
      if (!isEdgeValid(neighborDist,armstart_anglesV_rad, neighborNode->jointAng, map,x_size, y_size,numofDOFs))
      {
       // printf("Edge not Valid \n");
        continue;
      }
    ...
    findNeighbors(graph, closestNodesGoal, closestDistGoal,armgoal_anglesV_rad, epsilon*10,numofDOFs);
    for (int ss=0; ss < closestNodesGoal->size(); ss++)
    {
      neighborNode = (*closestNodesGoal)[ss];
      double neighborDist = (*closestDistGoal)[ss];
      if (!isEdgeValid(neighborDist,armgoal_anglesV_rad, neighborNode->jointAng, map,x_size, y_size,numofDOFs))
      {
        //printf("Edge not Valid \n");
        continue;
      }
    ...
    initNode->h_val = heuristicCalc(initNode->jointAng, armgoal_anglesV_rad,numofDOFs);
    visited[0] = true;
      ...
      for (int h=0; h < currNode->adjNodes->size(); h++)
      {
        // if the node has been expanded
          ...
          else 
          {
            ...
            adjNode->g_val = currNode->g_val + adjNode->cost;
            adjNode->h_val = heuristicCalc(adjNode->jointAng,armgoal_anglesV_rad,numofDOFs);
            adjNode->f_val = adjNode->g_val + searchWt*adjNode->h_val;
            adjNode->parent = currNode;
    ...
    while(1)
    {
      if (currNode->idx > 0)
      {
        ...
        pathQuality += distanceCalc(currNode->jointAng, currNode->parent->jointAng,numofDOFs);
    ...
    *plan = (double**) malloc((*planlength)*sizeof(double*));
    ...
    if (finalPath.empty())
    {
      ...
      printf("Path Quality for PRM Planner: %.2f\n", pathQuality);
    }
...
Node* findRRTNeighbor(vector<Node*>* graph,
  double &nearestDist, double* config, int armDOF)
{
  double dist = 0;
  nearestDist = INT_MAX;
  Node* nearestNode;
...
// Extends by epsilon
void steer(double* Xnearest, double* Xrand, double* newConfig, double nearestDist, double epsilon,int armDOF)
{
     ...
     )
{
    ...
    while (i < V)
    {
      ...
      if (i>100 && (i%100 ==0))
      {
        ...
        goalbiasConfig(armgoal_anglesV_rad, epsilon/4, &currConfig,numofDOFs);
      }
      else 
      {
        randomConfig(&currConfig,numofDOFs);
      ...
      nearestNode = findRRTNeighbor(graph, nearestDist,currConfig,numofDOFs);
      //printf("Neighbor search complete \n");
      // Extend the configuration towards q_rand by a distance epsilon
      if (nearestDist > epsilon)
      {
        for (int s = 0; s < armDOF; s++)
        {
          steer(nearestNode->jointAng, currConfig, newConfig, nearestDist, epsilon,numofDOFs);// newConfig[s] = nearestNode->jointAng[s] + ((newConfig[s] / newConfigMag) * epsilon) ;
        }
        //printf("Moved node by distance %.2f \n", distanceCalc(newConfig, nearestNode->jointAng));
      }

      double newDist = distanceCalc(newConfig,nearestNode->jointAng,numofDOFs);
      lastGoodConfig = NULL;
      if (!isEdgeValidRRT(newDist,nearestNode->jointAng,newConfig,lastGoodConfig, map,x_size, y_size,numofDOFs))
      {
      ...
      else if (i >= V-5)
      {
        ...
        goalNode = findRRTNeighbor(graph, goalNearestDist,armgoal_anglesV_rad,numofDOFs);
    ...
    while(closestToGoal->parent != NULL)
    {
      ...
      pathQuality += distanceCalc(closestToGoal->jointAng, closestToGoal->parent->jointAng,numofDOFs);
    ...
    printf("Path Quality for RRT Planner: %.2f\n", pathQuality);
    *planlength = finalPath.size();
    *plan = (double**) malloc((*planlength)*sizeof(double*));
    for (int m = 0; m < *planlength; m++){
        (*plan)[m] = (double*) malloc(numofDOFs*sizeof(double)); 
     ...
     )
{
    ...
    *planlength = 0;
    ...
    while(i<V)
    {
      ...
      {
        printf("************************Creating Bias Goal for iteration %d ************************\n", i);
        goalbiasConfig(armgoal_anglesV_rad, epsilon, &randConfig,numofDOFs);
      }
      else 
      {
        randomConfig(&randConfig,numofDOFs);
      }
      ...
      nearestNode = findRRTNeighbor(currGraph, nearestDist,randConfig,numofDOFs); // Find nearest node
      // move only by epsilon towards the sampled config
      steer(nearestNode->jointAng,randConfig,newConfigA,nearestDist, epsilon,numofDOFs);
      for (int k =0; k < armDOF; k++)
      {
        printf("After steer function, newConfigA is %.4f\n",newConfigA[k]);
      }
      double newConfigDist = distanceCalc(nearestNode->jointAng, newConfigA,numofDOFs);
      lastGoodConfig = NULL;
      if (!isEdgeValidRRT(newConfigDist,nearestNode->jointAng,newConfigA,lastGoodConfig,map,x_size, y_size,numofDOFs))
      {
        if (lastGoodConfig == NULL)
        {
          printf("NULL returned from edge validation \n");
      ...
      newConfigDist = distanceCalc(nearestNode->jointAng, newConfigA,numofDOFs); // update the distance if needed
      ...
      nearestNode = findRRTNeighbor(graphB, nearestDist,newConfigA,numofDOFs); // Find nearest node
      ...
      while(1)
      {
        steer(tempConfigNearestNode,newConfigA,newConfigB,nearestDist, steerDist,numofDOFs);// divide epsilon to make sure the targetConfig is not missed
        newConfigDist = distanceCalc(tempConfigNearestNode, newConfigB,numofDOFs);
        lastGoodConfig = NULL;
        if (!isEdgeValidRRT(newConfigDist,tempConfigNearestNode,newConfigB,lastGoodConfig,map,x_size, y_size,numofDOFs))
        {
          if (lastGoodConfig != NULL)
          {
            for (int k =0; k < armDOF; k++)
        ...
        distToMove -= distanceCalc(tempConfigNearestNode, newConfigB,numofDOFs); // calculate distance moved and subtract that from full distance
        if (distanceCalc(newConfigB, newConfigA,numofDOFs) == 0.00)
        {
      ...
      if (newConfigB != NULL)
      {
        ...
        newConfigDist = distanceCalc(nearestNode->jointAng, newConfigB,numofDOFs);
    ...
    while (1)
    {
      if ((backNode->parent->idx == 0) || (backNode->parent->idx == V))
      {
        ...
        pathQuality += distanceCalc(backNode->jointAng, backNode->parent->jointAng,numofDOFs);
        break;
      }
    ...
    printf("Path Quality for RRT Connect: %.2f \n", pathQuality);
	   ...
	   int* planlength)
{
  ...
  while (graph->size() < V)
  {
    ...
    {
      //printf("************************Creating Bias Goal for iteration %d ************************\n", i);
      goalbiasConfig(armgoal_anglesV_rad, epsilon/2, &randConfig,numofDOFs);
    }
    else 
    {
      randomConfig(&randConfig,numofDOFs);
    ...
    Node* nearestNode;
    double nearestDist = 0;
    nearestNode = findRRTNeighbor(graph,nearestDist, randConfig,numofDOFs);
    //printf("Nearest dist after neighbor search %.2f\n",nearestDist);
    // Steer function
    double* newConfig = (double*) malloc(sizeof(double)*armDOF);
    steer(nearestNode->jointAng, randConfig, newConfig, nearestDist, epsilon,numofDOFs);
    if (!IsValidArmConfiguration(newConfig, armDOF, map, x_size,y_size))
    {
      continue;
    }
  /*distStart = distanceCalc(randConfig, startConfig);
    distGoal  = distanceCalc(randConfig, goalConfig);

    if ((distStart + distGoal) > cBest)
    {
      printf("Distance for start and goal: %.2f and best cost is: %.2f\n",distStart+distGoal,cBest);
      printf("************************Node Rejection Activated*****************************\n");
      continue;
    }*/

    double newConfigDist = distanceCalc(newConfig, nearestNode->jointAng,numofDOFs);
    lastConfig = NULL;
    if (!isEdgeValidRRT(newConfigDist,nearestNode->jointAng, newConfig,lastConfig, map, x_size, y_size,numofDOFs))
    {
      if (lastConfig == NULL)
      {
        continue;
      ...
      for (int k =0; k < armDOF; k++)
      {
        newConfig[k] = lastConfig[k];
       // printf("Full Edge was not valid. The best case config returned was %.4f \n", lastGoodConfig[k]);
      }
    }

    // Sample nodes in the specified region
    double gammaDelta = 2.0;
    
    vector<Node*>* closestNodes = new vector<Node*>();
    vector<double>* closestDist = new vector<double>();
    rad = min(pow((gammaDelta)*(log(graph->size())/ graph->size()),(1/armDOF)), epsilon);
    findNeighbors(graph, closestNodes, closestDist, newConfig, epsilon,numofDOFs); // find neighbors of new config
    Node* adjNode;
    ...
    for (int ss=0; ss < closestNodes->size(); ss++)
    {
      ...
      if (!isEdgeValid(adjNodeDist, newConfig, adjNode->jointAng, map, x_size, y_size,numofDOFs))
      {
        continue;
      }
      if (minCost > (adjNode->cost + adjNodeDist))
      {
        ...
        bestNode  = adjNode;
        cBest = minCost;
        printf("Improving Cost\n");
      }
      else
      {
        //printf("Min Cost is: %.2f and current cost is: %.2f\n",minCost, adjNode->cost + adjNodeDist);
      }
    ...
    for (int ss=0; ss < closestNodes->size(); ss++)
    {
      adjNode = (*closestNodes)[ss];
      adjNodeDist = (*closestDist)[ss];
      if (!isEdgeValid(adjNodeDist, Xnew->jointAng, adjNode->jointAng, map, x_size, y_size,numofDOFs))
      {
        continue;
      }
    ...
    if (distanceCalc(newConfig, armgoal_anglesV_rad,numofDOFs) < epsilon/3)
    {
      printf("Goal Found \n");
      break;
    }
  ...
  double goalNearestDist = 0;
  double pathQuality = 0;
  Node* closestToGoal = findRRTNeighbor(graph, goalNearestDist,armgoal_anglesV_rad,numofDOFs);
  ...
  printf("Path Quality for RRT Star: %.2f\n", pathQuality);
